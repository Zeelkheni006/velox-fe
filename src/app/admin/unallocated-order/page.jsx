...
<li
  onClick={() =>
    router.push(
      `/admin/order-view/${order.id}?user_name=${encodeURIComponent(order.user_name)}&order_number=${order.order_number}&services=${encodeURIComponent(order.services)}&total_quantity=${order.total_quantity}&total_amount=${order.total_amount}&city=${order.city}&status=${order.status}`
    )
  }
>
